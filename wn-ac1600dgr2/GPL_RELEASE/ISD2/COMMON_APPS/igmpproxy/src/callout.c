...
void age_callout_queue(int elapsed_time) {
    ...
    for (ptr = queue; ptr; ptr = queue, i++) {
        if (ptr->time > elapsed_time) {
            ...
            queue = queue->next;
            IF_DEBUG igmplog(LOG_DEBUG, 0, "About to call timeout %d (#%d)", ptr->id, i);
...
int timer_nextTimer() {
    if (queue) {
        if (queue->time < 0) {
            igmplog(LOG_WARNING, 0, "timer_nextTimer top of queue says %d", 
                queue->time);
            return 0;
        }
        return queue->time;
...
int timer_setTimer(int delay, cfunc_t action, void *data) {
    ...
    if (node == 0) {
        igmplog(LOG_WARNING, 0, "Malloc Failed in timer_settimer\n");
        return -1;
    }
    node->func = action; 
    node->data = data;
    ...
    if (!queue) {
        ...
        while (ptr) {
            if (delay < ptr->time) {
                ...
                ptr->time -= node->time;
                IF_DEBUG {
                    igmplog(LOG_DEBUG, 0, "Created timeout %d (#%d) - delay %d secs", 
                        node->id, i, node->time);
...
int timer_clearTimer(int  timer_id) {
    ...
    while (ptr) {
        if (ptr->id == timer_id) {
            ...
            if (ptr->data)
                free(ptr->data);
            IF_DEBUG igmplog(LOG_DEBUG, 0, "deleted timer %d (#%d)", ptr->id, i);
            free(ptr);
            IF_DEBUG debugQueue();
            return 1;
        }
    ...
    IF_DEBUG {
        igmplog(LOG_DEBUG, 0, "failed to delete timer %d (#%d)", timer_id, i);
        debugQueue();
    }
    return 0;
}
...
static void debugQueue() {
    ...
    IF_DEBUG {
        for (ptr = queue; ptr; ptr = ptr->next) {
            igmplog(LOG_DEBUG, 0, "(Id:%d, Time:%d) ", ptr->id, ptr->time);
        }
    }
}
