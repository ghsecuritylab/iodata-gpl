...
void acceptIgmp(int recvlen) {
    ...
    struct Config *confmk = getCommonConfig();

    if (recvlen < sizeof(struct ip)) {
        igmplog(LOG_WARNING, 0,
    ...
    //IF_DEBUG igmplog(LOG_DEBUG, 0, "Got a IGMP request to process...");
    ...
    if (ip->ip_p == 0) {
        if (src == 0 || dst == 0) {
            igmplog(LOG_WARNING, 0, "kernel request not accurate");
        }
        else {
            ...
            if(checkVIF == 0) {
                igmplog(LOG_ERR, 0, "Upstream VIF was null.");
                return;
            } 
            else if(src == checkVIF->InAdr.s_addr) {
                igmplog(LOG_NOTICE, 0, "Route activation request from %s for %s is from myself. Ignoring.",
                    inetFmt(src, s1), inetFmt(dst, s2));
                return;
            }
            else if(!isAdressValidForIf(checkVIF, src) && confmk->no_alt_net == 0) {
                igmplog(LOG_WARNING, 0, "The source address %s for group %s, is not in any valid net for upstream VIF.",
                    inetFmt(src, s1), inetFmt(dst, s2));
                return;
            }
            
            // Activate the route.
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Route activate request from %s to %s",
                         inetFmt(src,s1), inetFmt(dst,s2));
    ...
    igmplog(LOG_NOTICE, 0, "RECV %s from %-15s to %s",
...
void sendIgmp(uint32 src, uint32 dst, int type, int code, uint32 group, int datalen) {
    ...
    if (sendto(MRouterFD, send_buf,
        ...
        if (errno == ENETDOWN)
            igmplog(LOG_ERR, errno, "Sender VIF was down.");
        else
            igmplog(LOG_INFO, errno,
                "sendto to %s on %s",
    ...
    IF_DEBUG igmplog(LOG_DEBUG, 0, "SENT %s from %-15s to %s",
