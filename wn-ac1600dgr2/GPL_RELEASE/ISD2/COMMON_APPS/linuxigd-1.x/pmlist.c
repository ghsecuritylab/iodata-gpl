...
int pmlist_AddPortMapping (int enabled, char *protocol, char *externalPort, char *internalClient, char *internalPort)
{
	...
	{
	  char dest[DEST_LEN];
#if 1 // 2011-10-05 Norkay, fix internalPort or externalPort has dummy "0" ex: 05004 
	  // iptables -t nat -I PREROUTING -i eth2.2 -p TCP --dport 05004 -j DNAT --to 192.168.0.2:05004
	  // iptables 1.3.8 will cause error (1.4.4 don't know)
	  char extPort[DEST_LEN];
	  char *args[] = {"iptables", "-t", "nat", "-I", g_vars.preroutingChainName, "-i", g_vars.extInterfaceName, "-p", protocol, "--dport", extPort, "-j", "DNAT", "--to", dest, NULL};
	  snprintf(dest, DEST_LEN, "%s:%d", internalClient, atoi(internalPort));
	  snprintf(extPort, DEST_LEN, "%d", atoi(externalPort));
	  snprintf(command, COMMAND_LEN, "%s -t nat -I %s -i %s -p %s --dport %d -j DNAT --to %s:%d", g_vars.iptables, g_vars.preroutingChainName, g_vars.extInterfaceName, protocol, atoi(externalPort), internalClient, atoi(internalPort));
#else	  
	...
	if (g_vars.forwardRules)
	{
	  ...
	  // iptables -t nat -I PREROUTING -i eth2.2 -p TCP --dport 05004 -j DNAT --to 192.168.0.2:05004
	  // iptables 1.3.8 will cause error (1.4.4 don't know)
	  char intPort[DEST_LEN];
	  char *args[] = {"iptables", "-A", g_vars.forwardChainName, "-p", protocol, "-d", internalClient, "--dport", intPort, "-j", "ACCEPT", NULL};
	  snprintf(intPort, DEST_LEN, "%d", atoi(internalPort));
	  snprintf(command, COMMAND_LEN, "%s -A %s -p %s -d %s --dport %d -j ACCEPT", g_vars.iptables,g_vars.forwardChainName, protocol, internalClient, atoi(internalPort));
#else	  
...
int pmlist_DeletePortMapping(int enabled, char *protocol, char *externalPort, char *internalClient, char *internalPort)
{
	...
	
	{
	  ...
	  // iptables 1.3.8 will cause error (1.4.4 don't know)
	  char extPort[DEST_LEN];
	  char *args[] = {"iptables", "-t", "nat", "-D", g_vars.preroutingChainName, "-i", g_vars.extInterfaceName, "-p", protocol, "--dport", extPort, "-j", "DNAT", "--to", dest, NULL};
      snprintf(dest, DEST_LEN, "%s:%d", internalClient, atoi(internalPort));
	  snprintf(extPort, DEST_LEN, "%d", atoi(externalPort));
	  snprintf(command, COMMAND_LEN, "%s -t nat -D %s -i %s -p %s --dport %d -j DNAT --to %s:%d",
		  g_vars.iptables, g_vars.preroutingChainName, g_vars.extInterfaceName, protocol, atoi(externalPort), internalClient, atoi(internalPort));
#else	  
	...
	if (g_vars.forwardRules)
	{
	  ...
	  // iptables 1.3.8 will cause error (1.4.4 don't know)
	  char intPort[DEST_LEN];
	  char *args[] = {"iptables", "-D", g_vars.forwardChainName, "-p", protocol, "-d", internalClient, "--dport", intPort, "-j", "ACCEPT", NULL};
	  snprintf(intPort, DEST_LEN, "%d", atoi(internalPort));
	  snprintf(command, COMMAND_LEN, "%s -D %s -p %s -d %s --dport %d -j ACCEPT", g_vars.iptables, g_vars.forwardChainName, protocol, internalClient, atoi(internalPort));
#else	  
