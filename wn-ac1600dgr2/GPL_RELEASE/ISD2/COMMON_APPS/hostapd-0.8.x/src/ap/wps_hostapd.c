...
#include "gconfig.h" //samhuang adds, 2012-0405 CONFIG_WPS2
...
{
#if 1/*jaykung to notify sysconfd*/
#define RTFILE_TAG  "/var/wirelessChangTag"
#endif

	const struct wps_credential *cred = ctx;
	FILE *oconf, *nconf,*mytag;
	size_t len, i;
	char *tmp_fname;
	char buf[1024];
	int multi_bss;
...
wpa_printf(MSG_INFO,"Sam: hostapd_init_wps");
	if (conf->wps_state == 0) {
	...
	if (wps == NULL)
		return -1;
	wps->disable_pin=conf->disable_pin;
...
#if HAS_SUPPORT_ATHEROS_RADIO2_WLAN //samhuang modified, 2012-0405 CONFIG_WPS2 for dualband case
	cfg.dualband = 1;
#else
	cfg.dualband = 0;//interface_count(hapd->iface) > 1;
#endif
	if (cfg.dualband)
...
//samhuang adds,2012-04-03, for hostapd-0.7.x
int hostapd_wps_stop(struct hostapd_data *hapd, const u8 *dev_addr)
{

	if (hapd->wps == NULL)
		return -1;
	/*stop LED*/
	/*stop pbc and pin methods*/
	return my_wps_stop(hapd->wps->registrar);
