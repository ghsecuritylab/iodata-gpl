...
#define MULTICAST_GROUP_FILE	"/tmp/multicast_group_file"
...
void initRouteTable() {
    ...
    for ( Ix = 0; Dp = getIfByIx( Ix ); Ix++ ) {
        ...
        // 2010/01/18 jerry chen: avoid igmpproxy sending "V3 membership report / join group" to upstream igmp interface
        if(strcmp(Dp->Name, extraInterface)!=0 && Dp->InAdr.s_addr && ! (Dp->Flags & IFF_LOOPBACK) && Dp->state == IF_STATE_DOWNSTREAM) {
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Joining all-routers group %s on vif %s",
...
void sendJoinLeaveUpstream(struct RouteTable* route, int join) {
    ...
    if(upstrIf == NULL) {
        igmplog(LOG_ERR, 0 ,"FATAL: Unable to get Upstream IF.");
    }
    /*
    IF_DEBUG {
        igmplog(LOG_DEBUG, 0, "Upstream IF addr  : %s", inetFmt(upstrIf->InAdr.s_addr,s1));
        igmplog(LOG_DEBUG, 0, "Upstream IF state : %d", upstrIf->state);
        igmplog(LOG_DEBUG, 0, "Upstream IF index : %d", upstrIf->index);
    }*/
    ...
    if(join) {
        ...
        if(route->vifBits > 0) {
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Joining group %s upstream on IF address %s",
            ...
            igmplog(LOG_DEBUG, 0, "No downstream listeners for group %s. No join sent.",
                inetFmt(route->group, s1));
        }
        ...
        if(route->upstrState != ROUTESTATE_NOTJOINED) {
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Leaving group %s upstream on IF address %s",
                         inetFmt(route->group, s1), 
                         inetFmt(upstrIf->InAdr.s_addr, s2));
...
void clearAllRoutes() {
    ...
    for(croute = routing_table; croute; croute = remainroute) {
        ...
        IF_DEBUG igmplog(LOG_DEBUG, 0, "Removing route entry for %s",
                     inetFmt(croute->group, s1));

        // Uninstall current route
        if(!internUpdateKernelRoute(croute, 0)) {
            igmplog(LOG_WARNING, 0, "The removal from Kernel failed.");
        }
    ...
    igmplog(LOG_NOTICE, 0, "All routes removed. Routing table is empty.");
}
...
int insertRoute(uint32 group, int ifx) {
    ...
    if( ! IN_MULTICAST( ntohl(group) )) {
        igmplog(LOG_WARNING, 0, "The group address %s is not a valid Multicast group. Table insert failed.",
            inetFmt(group, s1));
        return 0;
    }
    ...
    if(ifx >= MAX_MC_VIFS) {
        igmplog(LOG_WARNING, 0, "The VIF Ix %d is out of range (0-%d). Table insert failed.",ifx,MAX_MC_VIFS);
        return 0;
    }
    ...
    if(croute==NULL) {
        ...
        IF_DEBUG igmplog(LOG_DEBUG, 0, "No existing route for %s. Create new.",
                     inetFmt(group, s1));
        ...
        if(routing_table == NULL) {
            ...
            IF_DEBUG igmplog(LOG_DEBUG, 0, "No routes in table. Insert at beginning.");
        } else {

            IF_DEBUG igmplog(LOG_DEBUG, 0, "Found existing routes. Find insert location.");

            // Check if the route could be inserted at the beginning...
            if(routing_table->group > group) {
                IF_DEBUG igmplog(LOG_DEBUG, 0, "Inserting at beginning, before route %s",inetFmt(routing_table->group,s1));
                ...
                IF_DEBUG igmplog(LOG_DEBUG, 0, "Inserting after route %s",inetFmt(croute->group,s1));
        ...
        // Log the cleanup in debugmode...
        igmplog(LOG_INFO, 0, "Inserted route table entry for %s on VIF #%d",
        ...
        // Log the cleanup in debugmode...
        igmplog(LOG_INFO, 0, "Updated route entry for %s on VIF #%d",
            inetFmt(croute->group, s1), ifx);
        ...
        if(croute->originAddr != 0) {
            ...
            if(!internUpdateKernelRoute(croute, 1)) {
                igmplog(LOG_WARNING, 0, "The insertion into Kernel failed.");
                return 0;
            }
        }
    }
...
int activateRoute(uint32 group, uint32 originAddr) {
    ...
    croute = findRoute(group);
    if(croute == NULL) {
        IF_DEBUG igmplog(LOG_DEBUG, 0, "No table entry for %s [From: %s]. Inserting route.",
    ...
    if(croute != NULL) {
        ...
        if(originAddr > 0) {
            if(croute->originAddr > 0 && croute->originAddr!=originAddr) {
                igmplog(LOG_WARNING, 0, "The origin for route %s changed from %s to %s",
                    inetFmt(croute->group, s1),
...
void ageActiveRoutes() {
    ...
    IF_DEBUG igmplog(LOG_DEBUG, 0, "Aging routes in table.");
...
int removeRoute(struct RouteTable*  croute) {
    ...
    // Log the cleanup in debugmode...
    IF_DEBUG igmplog(LOG_DEBUG, 0, "Removed route entry for %s from table.",
                 inetFmt(croute->group, s1));
...
int internAgeRoute(struct RouteTable*  croute) {
    ...
    if(croute->ageValue == 0) {
        ...
        if(croute->ageActivity>0) {
            ...
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Updating route after aging : %s",
                         inetFmt(croute->group,s1));
            ...
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Removing group %s. Died of old age.",
                         inetFmt(croute->group,s1));
...
int internUpdateKernelRoute(struct RouteTable *route, int activate) {
    ...
    if(route->originAddr>0) {
        ...
        IF_DEBUG igmplog(LOG_DEBUG, 0, "Vif bits : 0x%08x", route->vifBits);

        // Set the TTL's for the route descriptor...
        for ( Ix = 0; Dp = getIfByIx( Ix ); Ix++ ) {
            if(Dp->state == IF_STATE_UPSTREAM) {
                //IF_DEBUG igmplog(LOG_DEBUG, 0, "Identified VIF #%d as upstream.", Dp->index);
                mrDesc.InVif = Dp->index;
            }
            else if(BIT_TST(route->vifBits, Dp->index)) {
                IF_DEBUG igmplog(LOG_DEBUG, 0, "Setting TTL for Vif %d to %d", Dp->index, Dp->threshold);
        ...
        igmplog(LOG_NOTICE, 0, "Route is not active. No kernel updates done.");
    }
...
void logRouteTable(char *header) {
    IF_DEBUG  {
        ...
        if(croute==NULL) {
            igmplog(LOG_DEBUG, 0, "No routes in table...");
        } else {
            do {
                /*
                igmplog(LOG_DEBUG, 0, "#%d: Src: %s, Dst: %s, Age:%d, St: %s, Prev: 0x%08x, T: 0x%08x, Next: 0x%08x",
                ...
                */
                igmplog(LOG_DEBUG, 0, "#%d: Src: %s, Dst: %s, Age:%d, St: %s, OutVifs: 0x%08x",
                    rcount, inetFmt(croute->originAddr, s1), inetFmt(croute->group, s2),
                    croute->ageValue,(croute->originAddr>0?"A":"I"),
                    croute->vifBits);
...
void getMulticastGroup()
{
	int count = 0;
	struct RouteTable*  croute = routing_table;
	char buf[512] = {0}, command[512] = {0};
	...
	if(croute)
	{
		do
		{
			...
			count += sprintf(buf+count, "%s", inetFmt(croute->group, s2));
		} while((croute = croute->nextroute) != NULL);
	}

	count += sprintf(buf+count, ";");

	sprintf(command, "echo \"%s\" > /tmp/multicast_group_file", buf);
	system(command);
