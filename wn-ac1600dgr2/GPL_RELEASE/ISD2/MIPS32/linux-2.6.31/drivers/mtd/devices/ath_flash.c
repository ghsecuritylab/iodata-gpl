...
#include "../sn_mtd/sn_partition_table.h"
...
#define ATH_FLASH_SECTOR_SIZE_32KB  (32*1024)
#define ATH_FLASH_SECTOR_SIZE_16KB  (16*1024)
#define ATH_FLASH_SECTOR_SIZE_8KB   (8*1024)
...
ath_flash_geom_t flash_geom_tbl[ATH_FLASH_MAX_BANKS] = {
        {
         ...
         .size = ATH_FLASH_SIZE_8MB,
#elif defined (CONFIG_MTD_FLASH_4MB)
         .size = ATH_FLASH_SIZE_4MB,
#elif defined (CONFIG_MTD_FLASH_2MB)
         .size = ATH_FLASH_SIZE_2MB,
#else
	#error "Please define CONFIG_MTD_FLASH"
#endif

#ifdef CONFIG_MTD_FLASH_SECTOR_SIZE_64KB
         .sector_size = ATH_FLASH_SECTOR_SIZE_64KB,
#elif defined (CONFIG_MTD_FLASH_SECTOR_SIZE_32KB)
        .sector_size = ATH_FLASH_SECTOR_SIZE_32KB,
#elif defined (CONFIG_MTD_FLASH_SECTOR_SIZE_16KB)
        .sector_size = ATH_FLASH_SECTOR_SIZE_16KB,
#elif defined (CONFIG_MTD_FLASH_SECTOR_SIZE_8KB)
        .sector_size = ATH_FLASH_SECTOR_SIZE_8KB,
#elif defined (CONFIG_MTD_FLASH_SECTOR_SIZE_4KB)
        .sector_size = ATH_FLASH_SECTOR_SIZE_4KB,
#else
	#error "Please define CONFIG_MTD_FLASH_SECTOR_SIZE"
#endif
         .pgsize = ATH_FLASH_PG_SIZE_256B}
};
...
ath_flash_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	do {
#ifdef CONFIG_MTD_FLASH_SECTOR_SIZE_64KB
                ath_spi_sector_erase(s_curr* ATH_FLASH_SECTOR_SIZE_64KB);
#elif defined (CONFIG_MTD_FLASH_SECTOR_SIZE_32KB)
                ath_spi_sector_erase(s_curr* ATH_FLASH_SECTOR_SIZE_32KB);
#elif defined (CONFIG_MTD_FLASH_SECTOR_SIZE_16KB)
                ath_spi_sector_erase(s_curr* ATH_FLASH_SECTOR_SIZE_16KB);
#elif defined (CONFIG_MTD_FLASH_SECTOR_SIZE_8KB)
                ath_spi_sector_erase(s_curr* ATH_FLASH_SECTOR_SIZE_8KB);
#elif defined (CONFIG_MTD_FLASH_SECTOR_SIZE_4KB)
                ath_spi_sector_erase(s_curr* ATH_FLASH_SECTOR_SIZE_4KB);
#else
                ath_spi_sector_erase(s_curr* ATH_FLASH_SECTOR_SIZE_64KB);
//                #error "Please define CONFIG_MTD_FLASH_SECTOR_SIZE"
#endif
...
#ifdef USER_DEFINITION_FLASH_MAP/* cfho 2009-0730, use pre-defined MTD partation table */

#if DYNAMIC_UDPATE_PARTITION_TBL
        sn_update_partition_tbl(mtd, mtd->erasesize, physmap_partitions_t, ARRAY_SIZE(physmap_partitions_t));
#endif
        add_mtd_partitions(mtd, physmap_partitions_t,
                                ARRAY_SIZE(physmap_partitions_t));
#else
...
#ifdef CONFIG_MTD_FLASH_SECTOR_SIZE_64KB
        ath_spi_bit_banger(ATH_SPI_CMD_SECTOR_ERASE);
#else   /* cfho 2010-0319, we use block erase, command is 0x20 */
        ath_spi_bit_banger(0x20);
#endif
