...
int loadConfig(char *configFile) {
    ...
    if(!openConfigFile(configFile)) {
        igmplog(LOG_ERR, 0, "Unable to open configfile from %s", configFile);
    }

    // Get first token...
    token = nextConfigToken();
    if(token == NULL) {
        igmplog(LOG_ERR, 0, "Config file was empty.");
    }
    ...
    while ( token != NULL ) {
        ...
        if(strcmp("phyint", token)==0) {
            ...
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Config: Got a phyint token.");
            tmpPtr = parsePhyintToken();
            if(tmpPtr == NULL) {
                // Unparsable token... Exit...
                closeConfigFile();
                igmplog(LOG_WARNING, 0, "Unknown token '%s' in configfile", token);
                return 0;
            } else {

                IF_DEBUG igmplog(LOG_DEBUG, 0, "IF name : %s", tmpPtr->name);
                IF_DEBUG igmplog(LOG_DEBUG, 0, "Next ptr : %x", tmpPtr->next);
                IF_DEBUG igmplog(LOG_DEBUG, 0, "Ratelimit : %d", tmpPtr->ratelimit);
                IF_DEBUG igmplog(LOG_DEBUG, 0, "Threshold : %d", tmpPtr->threshold);
                IF_DEBUG igmplog(LOG_DEBUG, 0, "State : %d", tmpPtr->state);
                IF_DEBUG igmplog(LOG_DEBUG, 0, "Allowednet ptr : %x", tmpPtr->allowednets);
            ...
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Config: Quick leave mode enabled.");
        ...
        else if(strcmp("no_alt_net", token)==0) {
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Config: No altnet used. Lekker puh....");
            commonConfig.no_alt_net = 1;
            
            // Read next token...
            token = nextConfigToken();
            continue;
        }
        else if(strcmp("upstream_igmp", token)==0) {
            // Got a interface for multicast upstream
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Config: Got a upstream_igmp token.");

            strcpy(extraInterface, nextConfigToken());
            hasUpstreamIgmpInf = 1;
        ...
        if ( Dp->InAdr.s_addr && ! (Dp->Flags & IFF_LOOPBACK) ) {
            ...
            for( confPtr = vifconf; confPtr; confPtr = confPtr->next) {
                ...
                if(strcmp(Dp->Name, confPtr->name)==0) {
                    ...
                    IF_DEBUG igmplog(LOG_DEBUG, 0, "Found config for %s", Dp->Name);
...
struct vifconfig *parsePhyintToken() {
    ...
    IF_DEBUG igmplog(LOG_DEBUG, 0, "Config: IF: Config for interface %s.", token);

    // Allocate memory for configuration...
    tmpPtr = (struct vifconfig*)malloc(sizeof(struct vifconfig));
    if(tmpPtr == NULL) {
        igmplog(LOG_ERR, 0, "Out of memory.");
    }
    ...
    while(token != NULL) {
        if(strcmp("altnet", token)==0) {
            ...
            token = nextConfigToken();
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Config: IF: Got altnet token %s.",token);

            *anetPtr = parseSubnetAddress(token);
            if(*anetPtr == NULL) {
                parseError = 1;
                igmplog(LOG_WARNING, 0, "Unable to parse subnet address.");
                break;
            } else {
            ...
            // Upstream
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Config: IF: Got upstream token.");
            tmpPtr->state = IF_STATE_UPSTREAM;
        }
        else if(strcmp("downstream", token)==0) {
            // Downstream
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Config: IF: Got downstream token.");
            tmpPtr->state = IF_STATE_DOWNSTREAM;
        }
        else if(strcmp("disabled", token)==0) {
            // Disabled
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Config: IF: Got disabled token.");
            tmpPtr->state = IF_STATE_DISABLED;
        }
        else if(strcmp("ratelimit", token)==0) {
            // Ratelimit
            token = nextConfigToken();
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Config: IF: Got ratelimit token '%s'.", token);
            tmpPtr->ratelimit = atoi( token );
            if(tmpPtr->ratelimit < 0) {
                igmplog(LOG_WARNING, 0, "Ratelimit must be 0 or more.");
                parseError = 1;
                break;
            }
        }
            ...
            // Threshold
            token = nextConfigToken();
            IF_DEBUG igmplog(LOG_DEBUG, 0, "Config: IF: Got threshold token '%s'.", token);
            tmpPtr->threshold = atoi( token );
            if(tmpPtr->threshold <= 0 || tmpPtr->threshold > 255) {
                igmplog(LOG_WARNING, 0, "Threshold must be between 1 and 255.");
                parseError = 1;
                break;
            }
        }
...
struct SubnetList *parseSubnetAddress(char *addrstr) {
    ...
    if(tmpStr != NULL) {
        ...
        if(bitcnt <= 0 || bitcnt > 32) {
            igmplog(LOG_WARNING, 0, "The bits part of the address is invalid : %d.",tmpStr);
            return NULL;
        }
    ...
    if(addr == -1 || addr == 0) {
        igmplog(LOG_WARNING, 0, "Unable to parse address token '%s'.", addrstr);
        return NULL;
    }
    ...
    IF_DEBUG igmplog(LOG_DEBUG, 0, "Config: IF: Altnet: Parsed altnet to %s.",
