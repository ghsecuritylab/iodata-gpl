                               ...
                               const char * desc_str)
{
	...
	r = UPNP_AddPortMapping(urls->controlURL, data->first.servicetype,
	                        eport, iport, iaddr, desc_str, proto, 0);
	if(r!=UPNPCOMMAND_SUCCESS)
...
int main(int argc, char ** argv)
{
...
#endif
#if 0 /* 2011-0121 Mook, disable banner message */
	...
	/* 2011-0121 Mook, force to use the same port number (1900) with SSDP protocol. */
	if( rootdescurl
	  || (devlist = upnpDiscover(2000, multicastif, minissdpdpath, 1)))
	{
		  ...
		  || (i = UPNP_GetValidIGD(devlist, &urls, &data, lanaddr, sizeof(lanaddr))))
		{
			...
			case 'a':
				SetRedirectAndTest(&urls, &data,
				                   ...
				                   commandargv[2], commandargv[3], ex_desc_str);
				break;
			case 'd':
			...
			case 'r':
				for(i=0; i<commandargc; i+=2)
				{
					...
					SetRedirectAndTest(&urls, &data,
									   ...
									   commandargv[i], commandargv[i+1], ex_desc_str);
				}
				break;
			default:
