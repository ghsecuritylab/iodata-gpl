...
#if HAS_USER_MANUAL_BOOTPFLAG
	if(server_config.bootp_type == BOOTP_FLAG_BCAST) 
	{
		packet->flags = htons(0x8000);
	}
	else if(server_config.bootp_type == BOOTP_FLAG_UCAST) 
	{
...
int sendOffer(struct dhcpMessage *oldpacket)
{
	...
	if (!static_lease_ip) {
		...
		if (lease && !reservedIp(server_config.static_leases, lease->yiaddr)) {
		...
		/* Or the client has a requested ip */
		 ...
		 && !reservedIp(server_config.static_leases, req_align) /* Check that its not a static lease */
		 ...
		 && (!(lease = find_lease_by_yiaddr(req_align))
            ...
            {
                packet.yiaddr = find_address(1); /* reserved for DHCP Server, find another one */
            }else
            {
			    if( 0==arpping(req_align, server_config.server, 
				    	server_config.arp, server_config.interface) )
			    {
				    /* get Arp, This IP was used, find another one */
	...
	bb_info_msg("Sending OFFER of %s", inet_ntoa(addr));
        /* systemlog message */
        SystemLog("DHCP Server, Sending OFFER of %s", inet_ntoa(addr));
...
int sendACK(struct dhcpMessage *oldpacket, uint32_t yiaddr, char * hostname)
{
	...
	bb_info_msg("Sending ACK to %s", inet_ntoa(addr));
        /* systemlog message */
        SystemLog("DHCP Server, Sending ACK of %s", inet_ntoa(addr));

#if DHCPD_REQUEST_INIC_CLIENTINFO
	// 2011-07-04 Norkay, Remove request clientinfo to udhcpd from https.
	system("sysconf_iNIC request clientinfo &");
#endif
