...
/*	s17 HNAT PPPoE need "pppoe session ID", "pppoe peer IP", "pppoe peer MAC", "pppoe wan MAC"
	Here we provide "PPPoE Session ID" for HWNAT use
*/
    {
        unsigned char buf[256] = "\0";

#ifndef HAS_ATHRS_S17C_CONFIG
        sprintf(buf, "[ -e \"/proc/sys/net/netfilter/nf_athrs17_hnat_ppp_id\" ] && echo %d > /proc/sys/net/netfilter/nf_athrs17_hnat_ppp_id", (int) ntohs(conn->session));
#else
        sprintf(buf, "[ -e \"/proc/qca_switch/nf_athrs17_hnat_ppp_id\" ] && echo %d > /proc/qca_switch/nf_athrs17_hnat_ppp_id", (int) ntohs(conn->session));
#endif
	system(buf);
    }
#endif
...
#ifdef _s17_hwaccel
/*	s17 HNAT PPPoE need "pppoe session ID", "pppoe peer IP", "pppoe peer MAC", "pppoe wan MAC"
	Here we provide "wan port mac" for HWNAT use
*/
	{
		unsigned char buf[256] = "\0";
#ifdef HAS_ATHRS_S17C_CONFIG
		sprintf(buf,"[ -e \"/proc/qca_switch/nf_athrs17_hnat_wan_mac\" ] && echo %02x%02x%02x%02x%02x%02x > /proc/qca_switch/nf_athrs17_hnat_wan_mac"
#else
		sprintf(buf,"[ -e \"/proc/sys/net/netfilter/nf_athrs17_hnat_wan_mac\" ] && echo %02x%02x%02x%02x%02x%02x > /proc/sys/net/netfilter/nf_athrs17_hnat_wan_mac"
#endif
			,conn->myEth[0], conn->myEth[1], conn->myEth[2]
			,conn->myEth[3], conn->myEth[4], conn->myEth[5]);
		system(buf);
	}
#endif
	...
	if (padiAttempts > MAX_PADI_ATTEMPTS) {
	    system("sysconf_cli ppp_inform ppp-pad0-timeout &");
	    if (persist) {
		padiAttempts = 0;
