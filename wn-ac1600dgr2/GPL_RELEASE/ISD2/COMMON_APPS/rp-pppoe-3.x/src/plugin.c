...
/*	s17 HNAT PPPoE need "pppoe session ID", "pppoe peer IP", "pppoe peer MAC", "pppoe wan MAC"
	Here we provide "peer mac" for HWNAT use
*/

	{
		unsigned char buf[256] = "\0";
		// write PPPoE peer mac,
#ifdef HAS_ATHRS_S17C_CONFIG
		sprintf(buf,"[ -e \"/proc/qca_switch/nf_athrs17_hnat_ppp_peer_mac\" ] && echo %02x%02x%02x%02x%02x%02x > /proc/qca_switch/nf_athrs17_hnat_ppp_peer_mac"
#else
		sprintf(buf,"[ -e \"/proc/sys/net/netfilter/nf_athrs17_hnat_ppp_peer_mac\" ] && echo %02x%02x%02x%02x%02x%02x > /proc/sys/net/netfilter/nf_athrs17_hnat_ppp_peer_mac"
#endif
...
PPPOEDisconnectDevice(void)
{
    ...
    /* Send PADT while disconnect PPPOE */
    sendPADT(conn, "PPPOE Disconnect");
...
PPPoEDevnameHook(char *cmd, char **argv, int doit)
{
	...
	if (strncmp(cmd, "eth", 3) && strncmp(cmd, "ixp", 3) &&
	    strncmp(cmd, "br", 2)) {
