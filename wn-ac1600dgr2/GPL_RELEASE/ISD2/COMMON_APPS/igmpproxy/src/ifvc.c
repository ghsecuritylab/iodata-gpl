...
void buildIfVc() {
    ...
    if ( (Sock = socket( AF_INET, SOCK_DGRAM, 0 )) < 0 )
        igmplog( LOG_ERR, errno, "RAW socket open" );
     ...
     */
    {
        ...
        if ( ioctl( Sock, SIOCGIFCONF, &IoCtlReq ) < 0 )
            igmplog( LOG_ERR, errno, "ioctl SIOCGIFCONF" );
     ...
     */
    {
        ...
        for ( IfPt = IfVc; IfPt < IfEp; IfPt++ ) {
            ...
            if (ioctl(Sock, SIOCGIFNETMASK, &IfReq ) < 0)
                igmplog(LOG_ERR, errno, "ioctl SIOCGIFNETMASK for %s", IfReq.ifr_name);
            mask = ((struct sockaddr_in *)&IfReq.ifr_addr)->sin_addr.s_addr;
            subnet = addr & mask;

            // Get the physical index of the Interface
            if (ioctl(Sock, SIOCGIFINDEX, &IfReq ) < 0)
                igmplog(LOG_ERR, errno, "ioctl SIOCGIFINDEX for %s", IfReq.ifr_name);
            
            igmplog(LOG_DEBUG, 0, "Physical Index value of IF '%s' is %d",
            ...
            */
            if ( ioctl( Sock, SIOCGIFFLAGS, &IfReq ) < 0 )
                igmplog( LOG_ERR, errno, "ioctl SIOCGIFFLAGS" );
            ...
            if(IfDescEp->allowednets == NULL) igmplog(LOG_ERR, 0, "Out of memory !");
            ...
            IF_DEBUG igmplog( LOG_DEBUG, 0, "buildIfVc: Interface %s Addr: %s, Flags: 0x%04x, Network: %s",
                 IfDescEp->Name,
...
int isAdressValidForIf( struct IfDesc* intrface, uint32 ipaddr ) {
    ...
    for(currsubnet = intrface->allowednets; currsubnet != NULL; currsubnet = currsubnet->next) {
        ...
        /*
        IF_DEBUG igmplog(LOG_DEBUG, 0, "Testing %s for subnet %s, mask %s: Result net: %s",
            inetFmt(ipaddr, s1),
