...
//#define CONFIG_IP_CONNTRACK_NAT_SESSION_RESERVATION 1
#ifdef CONFIG_IP_CONNTRACK_NAT_SESSION_RESERVATION
#define HAS_IP_CONNTRACK_NAT_SESSION_RESERVATION 1
#define NEW_CONNTRACK_DROP_SELECT                1
#endif
...
#if HAS_IP_CONNTRACK_NAT_SESSION_RESERVATION
int nf_conntrack_max_0 __read_mostly;
int nf_conntrack_max_reserved;
//int (*dropWhenNatTableFull_Ptr)(struct sk_buff *skb);
...
//unsigned char nf_athrs17_hnat_wan_ip[ATHRS17_PEER_IP_LEN];
...
//EXPORT_SYMBOL(nf_athrs17_hnat_wan_ip);
		  ...
		  enum ip_conntrack_info *ctinfo)
{
	...
	if (!h) {
#if HAS_IP_CONNTRACK_NAT_SESSION_RESERVATION
        // printk(">>> nf_conntrack_count %d nf_conntrack_max %d,max0 %d\n", nf_conntrack_count, nf_conntrack_max,nf_conntrack_max_0);
        if (dropWhenNatTableFull_Ptr)
        {
            if (atomic_read(&net->ct.count)> nf_conntrack_max_0)
            {
                //printk("==> NAT sessions full!\n");
                if (dropWhenNatTableFull_Ptr(skb,atomic_read(&net->ct.count),nf_conntrack_max))
                {       
                    //   printk("Drop!\n");
                    return NULL;
                }
...
static int nf_conntrack_init_init_net(void)
{
	//int max_factor = 8;
	int ret;
#if 1 //john.huang 2013-0125
#ifdef MAX_NAT_SESSION_NUM
    nf_conntrack_htable_size = MAX_NAT_SESSION_NUM/8; /*max is MAX_NAT_SESSION_NUM*/
#else
    nf_conntrack_htable_size = 256; /*max is 8*256=2048*/
#endif // MAX_NAT_SESSION_NUM //
#else
...
#if HAS_IP_CONNTRACK_NAT_SESSION_RESERVATION
    nf_conntrack_max_reserved = 200;
    nf_conntrack_max = 8 * nf_conntrack_htable_size;
    nf_conntrack_max_0 = nf_conntrack_max - nf_conntrack_max_reserved;
#else
    //nf_conntrack_max = max_factor * nf_conntrack_htable_size;
    nf_conntrack_max = 8 * nf_conntrack_htable_size;
#endif // HAS_IP_CONNTRACK_NAT_SESSION_RESERVATION //

#if HAS_IP_CONNTRACK_NAT_SESSION_RESERVATION
        printk("ip_conntrack version %s (%u buckets, %d max %d max0 %d reserved)"
               " - %Zd bytes per conntrack\n", 
               NF_CONNTRACK_VERSION,nf_conntrack_htable_size,
               nf_conntrack_max, nf_conntrack_max_0, nf_conntrack_max_reserved,
               sizeof(struct nf_conntrack));
#else
