...
int get_current_freq(void)
{
	 ...
	 */
/*Adonn 07-1123: option -l can't use*/
#if 0 //cfho 2007-1115
//#ifdef linux
	struct timex txc;
	txc.modes=0;
...
#undef Data
	...
	if (set_clock) {   /* you'd better be root, or ntpclient will crash! */
		...
		if(tv_set.tv_sec-preSec<=3) /*Adonn 08-0905: don't update system time in 3 secs*/
		{
			goto DEBUG;
		}
		else
		{
			preSec = tv_set.tv_sec;
		}

        if(g_set_timezone)
        {
            real_tz.tz_minuteswest = g_timezone;
            real_tz.tz_dsttime = 0;
            /* use real_tz as tz */
            tz = &real_tz;
        }
		...
		{
			/*When updating system is ok, it will write 0 to /var/tmp/ntp_result file and 
			update system time by time zone and daylight saving*/
			system("echo 0 > /var/tmp/ntp_result");
			system("echo 1 > /var/tmp/ntp_time_once");
			//system("sysconf_cli inform update_time");
			system("sysconf_cli ntpmanager tzUpdate &");
		}
...
int primary_loop(int usd, int num_probes, int interval, int goodness)
{
	...
	for (;;) {
		...
		if (pack_len<0) {
			...
			/* Adonn 07-1113: When setting time is ok, return 0*/
			if(set_clock)
				return 0;
...
int main(int argc, char *argv[]) {
	...
	ret = primary_loop(usd, probe_count, cycle_time, goodness);
