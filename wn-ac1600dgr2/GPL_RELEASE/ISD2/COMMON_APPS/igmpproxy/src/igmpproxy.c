...
int main( int ArgCn, const char *ArgVc[] ) {
        ...
        if ( strlen(ArgVc[i]) > 1 && ArgVc[i][0] == '-') {
            ...
            case 'c':
                ...
                if (i + 1 < ArgCn && ArgVc[i+1][0] != '-') {
                    ...
                    igmplog(LOG_ERR, 0, "Missing config file path after -c option.");
                }
                break;
            }
        }
    ...
    IF_DEBUG igmplog(LOG_DEBUG, 0, "Searching for config file at '%s'" , configFilePath);

    do {

        // Loads the config file...
        if( ! loadConfig( configFilePath ) ) {
            igmplog(LOG_ERR, 0, "Unable to load config file...");
            break;
        }
    
        // Initializes the deamon.
        if ( !igmpProxyInit() ) {
            igmplog(LOG_ERR, 0, "Unable to initialize IGMPproxy.");
            break;
        }
        ...
        if ( ! debugMode ) {
            ...
            IF_DEBUG igmplog( LOG_DEBUG, 0, "Starting daemon mode.");
            ...
            if ( close( 0 ) < 0 || close( 1 ) < 0 || close( 2 ) < 0 
                ...
                igmplog( LOG_ERR, errno, "failed to detach deamon" );
            }
        }
    ...
    igmplog(LOG_INFO, 0, "Shutdown complete....");
...
int igmpProxyInit() {
    ...
    case 0: break;
    case EADDRINUSE: igmplog( LOG_ERR, EADDRINUSE, "MC-Router API already in use" ); break;
    default: igmplog( LOG_ERR, Err, "MRT_INIT failed" );
    }
     ...
     */
    {
            ...
            if ( Dp->InAdr.s_addr && ! (Dp->Flags & IFF_LOOPBACK) ) {
                if(Dp->state == IF_STATE_UPSTREAM) {
                    if(upStreamVif == -1) {
                        ...
                        igmplog(LOG_ERR, 0, "Vif #%d was already upstream. Cannot set VIF #%d as upstream as well.",
                            upStreamVif, Ix);
                    }
                }
        ...
        if(vifcount < 2 || upStreamVif < 0) {
            igmplog(LOG_ERR, 0, "There must be at least 2 Vif's where one is upstream.");
        }
    }  
...
void igmpProxyCleanUp() {
    ...
    igmplog( LOG_DEBUG, 0, "clean handler called" );
    ...
    for (;;) {
        ...
        if (sighandled) {
            if (sighandled & GOT_SIGINT) {
                ...
                igmplog(LOG_NOTICE, 0, "Got a interupt signal. Exiting.");
                break;
            }
	    else if (sighandled & GOT_SIGUSR2) {
	        sighandled &= ~GOT_SIGUSR2;
	        getMulticastGroup();
                igmplog(LOG_NOTICE, 0, "Got SIGUSR2 signal");
	    }
        }
        ...
        if( Rt < 0 ) {
            igmplog( LOG_WARNING, errno, "select() failure" );
            continue;
        }
        else if( Rt > 0 ) {
            ...
            if( FD_ISSET( MRouterFD, &ReadFDS ) ) {
                ...
                recvlen = recvfrom(MRouterFD, recv_buf, RECV_BUF_SIZE,
                                   0, NULL, &dummy);
                if (recvlen < 0) {
                    if (errno != EINTR) igmplog(LOG_ERR, errno, "recvfrom");
                    continue;
                }
