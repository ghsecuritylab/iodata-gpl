...
int wpas_wps_eapol_cb(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WPS && !wpa_s->wps_success)
	{
		...
		system("sysconf_cli inform wps-led-fail&");
	}
...
#if 1 /* Senao patch */
#if SUPPORT_MRHESS
    /* 20131028 leonard: select which radio connected successfully. */
    if (strcmp(wpa_s->ifname, ATH2_REPEATER_DEV)==0)
    {
        system("sysconf_cli inform sel_radio2_wps_pbc &");
    }
    else
    {
        system("sysconf_cli inform sel_radio1_wps_pbc &");
    }
#endif
	system("sysconf_cli update_wpaconf2apcfg");
#endif
}
