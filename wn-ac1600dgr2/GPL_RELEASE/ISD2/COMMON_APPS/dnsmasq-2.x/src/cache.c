...
#if HAS_DNSMASQ_DNSREPLY_TO_SYSCONF_CLI
static int check_insert_times=0;
static char fitst_query_domain[512]={0};
#endif
...
char* Ipint2str(const int ip)
{
	static char emptyValue[24];

	sprintf(emptyValue,IP_PATTERN,
			(ip>>24)&0xff,
			(ip>>16)&0xff,
			(ip>>8)&0xff,
			(ip)&0xff);
...
#if HAS_DNSMASQ_DNSREPLY_TO_SYSCONF_CLI
	if(enable_dnsreply_to_sysconfcli == 1)
	{
/*
[/root]$nslookup munchkin.marketo.net
Server:		10.0.200.182
Address:	10.0.200.182#53

Non-authoritative answer:
munchkin.marketo.net	canonical name = marketo.hs.llnwd.net.
Name:	marketo.hs.llnwd.net
Address: 203.77.187.152

When Browser URL input: http://munchkin.marketo.net
cache_insert function will do twice as follow:

Debug msg:
jason debug ,cache_insert[539], --2---[munchkin.marketo.net]----[0.0.0.0]--
jason debug ,cache_insert[505], ----2--------[marketo.hs.llnwd.net]----[203.77.187.152]--

So we use fitst_query_domain to save [munchkin.marketo.net] bind [203.77.187.152] for sysconf_cli use.
*/
		//20120605 Jason: add for MultiPPPoE dynamic routing table function
		check_insert_times++; //insert time	
		if(addr)
		{
			if(check_insert_times == 1)
			{
				sprintf(cmd, "sysconf_cli dnsmasq_reply %s %s &", cache_get_name(new), inet_ntoa(new->addr.addr.addr.addr4));

				system(cmd);
				check_insert_times = 0; //reset
			}
			else
			{
				sprintf(cmd, "sysconf_cli dnsmasq_reply %s %s &", fitst_query_domain, inet_ntoa(new->addr.addr.addr.addr4));
				system(cmd);
				check_insert_times = 0; //reset
			}
		}
		else
		{
			if(check_insert_times == 1)
			{
				sprintf(fitst_query_domain, cache_get_name(new));
			}
		}
	}
#endif
