...
#ifdef ATHEROS_SUPPORT_WPS_LED_SIMPLE_CONFIG
#define WPS_LED_STATUS  "/proc/simple_config/simple_config_led"
...
static void wps_registrar_set_selected_timeout(void *eloop_ctx,
					       void *timeout_ctx);
/* +++ samhuang add */
static void wps_registrar_pbc_progress_stop(void *eloop_ctx, void *timeout_ctx);
static void wps_registrar_set_selected_progress_stop(void *eloop_ctx,void *timeout_ctx);
/* --- */
...
#if 0 //Adonn 11-0314: For pbc of Win 7 logo
	methods &= ~WPS_CONFIG_PUSHBUTTON;
#ifdef CONFIG_WPS2
	methods &= ~(WPS_CONFIG_VIRT_PUSHBUTTON |
		     WPS_CONFIG_PHY_PUSHBUTTON);
...
static void wps_registrar_pbc_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	sprintf(command, "echo 1 >%s",WPS_LED_STATUS);
	system(command);
#else
	if (reg->wps->dev.rf_bands == WPS_RF_24GHZ) {
		char    command[READ_BUF_SIZE];
		//3->wps_blinking 4->wps_fail 5->wps_success
		//sprintf(command, "echo 4 >%s",WPS_LED_STATUS);
        // LED_ACTION_WPS_FAIL = 14,
        // LED_ACTION_WPS_PASS = 15,
		sprintf(command, "echo 0 %d 14 >%s",SENAO_GPIO_LED_WPS, WPS_LED_STATUS);
		system(command);
	}
#if HAS_SUPPORT_ATHEROS_RADIO2_WLAN
	else if (reg->wps->dev.rf_bands == WPS_RF_50GHZ) {
		char    command[READ_BUF_SIZE];
		//3->wps_blinking 4->wps_fail(fast blinking) 5->wps_success
        // LED_ACTION_WPS_FAIL = 14,
        // LED_ACTION_WPS_PASS = 15,
#ifdef SENAO_GPIO_LED_WPS2
		sprintf(command, "echo 0 %d 14 >%s",SENAO_GPIO_LED_WPS2, WPS_LED_STATUS);
#endif
		system(command);
	}
#endif
...
static void wps_registrar_pbc_progress_stop(void *eloop_ctx, void *timeout_ctx)
{
	...
	//printf( "WPS:radio%d PBC Stop - disable PBC mode",reg->wps->dev.rf_bands);
	wps_pbc_timeout_event(reg->wps);
	wps_registrar_stop_pbc(reg);
}
...
//samhuang adds, 2012-0403
int my_wps_stop(struct wps_registrar *reg)
{
	/*stop pbc*/
	eloop_cancel_timeout(wps_registrar_pbc_progress_stop, reg, NULL);
	eloop_register_timeout(0, 0, wps_registrar_pbc_progress_stop,
			       reg, NULL);
	/*stop pin*/
	eloop_cancel_timeout(wps_registrar_set_selected_progress_stop, reg, NULL);
	eloop_register_timeout(0, 0,
			       wps_registrar_set_selected_progress_stop,
			       reg, NULL);
...
int wps_registrar_pbc_status(struct wps_registrar *reg)
{
	return reg->pbc;
}
...
static void wps_registrar_set_selected_progress_stop(void *eloop_ctx,
					       void *timeout_ctx)
{
	struct wps_registrar *reg = eloop_ctx;

	//printf("WPS: PIN is set - cancelling it\n");
	wps_registrar_pin_completed(reg);
	wps_registrar_invalidate_wildcard_pin(reg);
}
