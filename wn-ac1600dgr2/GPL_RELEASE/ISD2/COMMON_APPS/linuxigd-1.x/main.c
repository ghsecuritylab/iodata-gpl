...
int gAllowToSetIptables = 1; //adam: 2008-11-28
...
int main (int argc, char** argv)
{
	...
	if (argc < 3 || argc > 5) {
	  ...
	  printf("Usage: upnpd [-f] [-r] <external ifname> <internal ifname>\n");
	  printf("  -f\tdon't daemonize\n");
	  printf("  -r\tdon't set iptable rules\n");
...
#else
        printf ("Error Initializing UPnP SDK on IP %s \n",intIpAddress);
        printf ("  UpnpInit returned %d\n", ret);
#endif
		UpnpFinish();
		exit(1);
	}
...
#else
		printf ( "Error Setting Web Server Root Directory to: %s\n", g_vars.xmlPath);
        printf ("  UpnpSetWebServerRootDir returned %d\n", ret); 
#endif
		UpnpFinish();
		exit(1);
	}
...
#else
	  printf("ExpirationTimerInit failed\n");
#endif
	  UpnpFinish();
	  exit(1);
	}
...
#else
        printf("Error registering the root device with descDocUrl: %s\n", descDocUrl);
        printf("  UpnpRegisterRootDevice returned %d\n", ret);
#endif
		UpnpFinish();
		exit(1);
	}
