...
void k_set_rcvbuf(int bufsize, int minsize) {
    ...
    if (setsockopt(MRouterFD, SOL_SOCKET, SO_RCVBUF,
        ...
        if (bufsize < minsize) {
            igmplog(LOG_ERR, 0, "OS-allowed buffer size %u < app min %u",
                bufsize, minsize);
            /*NOTREACHED*/
        }
    }
    IF_DEBUG igmplog(LOG_DEBUG, 0, "Got %d byte buffer size in %d iterations", bufsize, iter);
}
...
void k_hdr_include(int bool) {
    ...
    if (setsockopt(MRouterFD, IPPROTO_IP, IP_HDRINCL,
        ...
        igmplog(LOG_ERR, errno, "setsockopt IP_HDRINCL %u", bool);
#endif
}
...
void k_set_ttl(int t) {
    ...
    if (setsockopt(MRouterFD, IPPROTO_IP, IP_MULTICAST_TTL,
        ...
        igmplog(LOG_ERR, errno, "setsockopt IP_MULTICAST_TTL %u", ttl);
#endif
    curttl = t;
}
...
void k_set_loop(int l) {
    ...
    if (setsockopt(MRouterFD, IPPROTO_IP, IP_MULTICAST_LOOP,
        ...
        igmplog(LOG_ERR, errno, "setsockopt IP_MULTICAST_LOOP %u", loop);
}
...
void k_set_if(uint32 ifa) {
    ...
    if (setsockopt(MRouterFD, IPPROTO_IP, IP_MULTICAST_IF,
        ...
        igmplog(LOG_ERR, errno, "setsockopt IP_MULTICAST_IF %s",
            inetFmt(ifa, s1));
}
...
void k_join(uint32 grp, uint32 ifa) {
    ...
    if (setsockopt(MRouterFD, IPPROTO_IP, IP_ADD_MEMBERSHIP,
        ...
        igmplog(LOG_WARNING, errno, "can't join group %s on interface %s",
...
void k_leave(uint32 grp, uint32 ifa) {
    ...
    if (setsockopt(MRouterFD, IPPROTO_IP, IP_DROP_MEMBERSHIP,
                   (char *)&mreq, sizeof(mreq)) < 0)
        igmplog(LOG_WARNING, errno, "can't leave group %s on interface %s",
            inetFmt(grp, s1), inetFmt(ifa, s2));
}
*/
