					 ...
					 char *buf, size_t buflen)
{
	...
	ret = os_snprintf(pos, end - pos, "wps_pbc_status=%s\n",
					  wps_registrar_pbc_status(hapd->wps->registrar) == 0
					  ? "0" : "1" );
	if (ret < 0 || ret >= end - pos)
		return pos - buf;
	pos += ret;
				       ...
				       void *sock_ctx)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		if (hostapd_wps_button_pushed(hapd, NULL))
			reply_len = -1;
	} else if (os_strcmp(buf, "WPS_STOP") == 0) {
		if (hostapd_wps_stop(hapd,(void *)NULL))
			reply_len = -1;
#ifdef CONFIG_WPS_OOB
