...
/* 2011-0412 mook, search kernel include path, but not toolchain. */
...
static option_t Options[] =
{
    ...
    { "pptp_phone", o_string, &pptp_phone,
      "PPTP Phone number" },
#if LINUX_VER_CODE <= KERNEL_VERSION_NUM(2,6,34)
...
/* [2010-12-28 Andy Yu]: Send up/down and ppp interface data to pptpd */
static void pptp_ppp_updown_ind(int up)
{
	char cmdbuf[128];       /* [2010-12-28 Andy Yu] */
	
	/* [2010-12-28 Andy Yu]:Record user log in information */
	syslog(LOG_INFO, "CTRL: Client %s log%s", pptp_client, up ? "in" : "out");
	snprintf(cmdbuf, sizeof(cmdbuf)-1, "sysconf_cli inform pptp-ppp-updown unit=%d:%d user=%s:%s",
			 ifunit, up, "---", pptp_client ? pptp_client : "0.0.0.0");
	system(cmdbuf);
}

/* [2010-12-28 Andy Yu]: Add up/down functions to handle up/down events coming from pppd */
static void pppopptp_ip_up_hook(void)
{
	pptp_ppp_updown_ind(1);

}

static void pppopptp_ip_down_hook(void)
{
	pptp_ppp_updown_ind(0);
}
...
void plugin_init(void)
{
	...
	/* [2010-12-28 Andy Yu]: Add up/down functions to handle up/down events coming from pppd */
	ip_up_hook = pppopptp_ip_up_hook;
	ip_down_hook = pppopptp_ip_down_hook;
}
