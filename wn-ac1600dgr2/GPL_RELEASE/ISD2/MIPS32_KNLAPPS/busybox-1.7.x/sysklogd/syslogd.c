...
#include "../../MIPS32_APPS/logmanager/logmanager.h"
#endif
...
#if HAS_PARENTAL_CONTROL
/* LOG_MAXLINE is defined in logmanager.h */
enum { MAX_READ = LOG_MAXLINE };
#else
enum { MAX_READ = 256 };
#endif
...
enum {
	...
	OPT_mask        = 1 << OPTBIT_mask    ,
	OPT_logsize     = 1 << OPTBIT_logsize ,
...
#define OPTION_STR "m:nO:l:k:z:S" \
...
#define OPTION_DECL *opt_m, *opt_l, *opt_k, *opt_z \
...
#define OPTION_PARAM &opt_m, &G.logFilePath, &opt_l, &opt_k, &opt_z \
	USE_FEATURE_ROTATE_LOGFILE(,&opt_s) \
	USE_FEATURE_ROTATE_LOGFILE(,&opt_b) \
	USE_FEATURE_REMOTE_LOG(    ,&opt_R) \
...
#define LOG_SIZE_MIN 16*1024
#define LOG_SIZE_MAX 64*1024
static int doLogSize = LOG_SIZE_MIN;
...
static void log_locally(char *msg)
{
 ...
 reopen:
    ...
    if(statf.st_size > doLogSize)
    {
        FILE *pf; 
        close(G.logFD);

        /* delete file */
        pf = fopen(G.logFilePath, "w");
        fclose(pf);
    }
#endif
...
#if ENABLE_FEATURE_REMOTE_LOG
#define SYSLOG_PRIORITY 0
#include "../../../MIPS32_APPS/logmanager/logmanager.h"
#define MAXLINE         1024    /* maximum line length */
static int doRemoteLogMask = 0;

static struct log_element logdata;

/*****************************************************************
* NAME: logPassing
* ---------------------------------------------------------------
* FUNCTION:  pass log message
* INPUT:
* OUTPUT:
* Author:   John Chang
* Modify:
****************************************************************/
static int logPassing(struct log_element *plogdata, char *msg, int len)
{
    int s, i;
    struct sockaddr_un remote;
    char *p, *pdate, *ptool, *pmsg;
    unsigned char *line;

    line=&(plogdata->data[0]);
    plogdata->len=0;
    plogdata->logger=-1;
    ...
    /*printf("logPassing:mag= %s\n", p);*/

    for(i=0; i<LOG_LOGGER_COUNT; i++)
    {
        if (0 == strncmp( ptool, logger_info[i].daemon, strlen(logger_info[i].daemon) ) )
        {
           /* replace Linux tool name to our defination */
           memset(line, 0x0, LOG_MAXLINE );
           /* sprintf(line,"%s, %s", logger_info[i].name, pmsg); */
           /* copy priority, timestamp */
           strncpy(line, msg, ptool-msg);
           
           /* add tool name and message body */
           sprintf(line+(ptool-msg),"%s: %s", logger_info[i].name, pmsg);
           
           plogdata->logger=logger_info[i].id;
           plogdata->len=strlen(line);
           
           break;
        }
    }
    /*printf("logPassing:line= %s\n", line);*/

    /* Can't find and do not need to send out */
    if(LOG_LOGGER_COUNT==i)
       return -1;
    ...
    strcpy(remote.sun_path, LOG_SOCK_PATH);
    ...
    len = sizeof(struct log_element);
    
    if (send(s, plogdata, len, 0) == -1)
    {
        perror("send");
        goto logPassingEnd;
    }
...
logPassingEnd:
    ...
    plogdata->len=0;
    /* Check the Remote syslog Mask */
    if(doRemoteLogMask&logger_info[i].mask)
    {
        plogdata->len=strlen(line);
        return 1;
    }
...
static void do_syslogd(void)
{
	...
	for (;;) {
		...
		if (FD_ISSET(sock_fd, &fds)) {
			...
			G.recvbuf[i] = '\0';
			logPassing(&logdata, G.recvbuf, i);
#if ENABLE_FEATURE_REMOTE_LOG
			...
			if (G.remoteAddr) {
				...
				if (-1 != G.remoteFD) {
					...
					sendto(G.remoteFD, &(logdata.data[0]), logdata.len, MSG_DONTWAIT,
						&G.remoteAddr->sa, G.remoteAddr->len);
				}
			    }
			}
...
#endif
	if (option_mask32 & OPT_mask) // -k
	{
		doRemoteLogMask = xatou_range(opt_k, 0, 0x100);
		printf("mask = %x\n", doRemoteLogMask);
	}

	if (option_mask32 & OPT_logsize) // -z
	{
		doLogSize = xatou_range(opt_z, 0, LOG_SIZE_MAX+1);
		if(doLogSize<LOG_SIZE_MIN)
		    doLogSize = LOG_SIZE_MIN;
		else if(doLogSize>LOG_SIZE_MAX)
		    doLogSize = LOG_SIZE_MAX;
