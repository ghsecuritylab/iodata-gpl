		    ...
		    u16 config_error, u16 error_indication)
{
	...
	system(command);
#else
	if (wps->dev.rf_bands == WPS_RF_24GHZ) {
		char    command[READ_BUF_SIZE];
	...
	else if (wps->dev.rf_bands == WPS_RF_50GHZ) {
		char    command[READ_BUF_SIZE];
...
void wps_success_event(struct wps_context *wps)
{
	...
	if (wps->dev.rf_bands == WPS_RF_24GHZ) {
		char    command[READ_BUF_SIZE];
		//3->wps_blinking 4->wps_fail 5->wps_success
		//sprintf(command, "echo 5 >%s",WPS_LED_STATUS);
        // LED_ACTION_WPS_FAIL = 14,
        // LED_ACTION_WPS_PASS = 15,
		sprintf(command, "echo 0 %d 15 >%s",SENAO_GPIO_LED_WPS, WPS_LED_STATUS);
		system(command);
	}
#if HAS_SUPPORT_ATHEROS_RADIO2_WLAN
	else if (wps->dev.rf_bands == WPS_RF_50GHZ) {
        ...
        // LED_ACTION_WPS_PASS = 15,
#ifdef SENAO_GPIO_LED_WPS2
		sprintf(command, "echo 0 %d 15 >%s",SENAO_GPIO_LED_WPS2, WPS_LED_STATUS);
#endif
		system(command);
	}
#endif
