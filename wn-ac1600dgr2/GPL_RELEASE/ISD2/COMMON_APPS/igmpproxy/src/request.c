...
extern struct RouteTable *findRoute(uint32 group);
// Prototypes...
...
uint32 Ipstr2int(const char *ipstr)
{
	...
	if(sscanf(ipstr,"%d.%d.%d.%d", &reg3, &reg2, &reg1, &reg0) != 4)
	{
...
void acceptGroupReport(uint32 src, uint32 group, uint8 type) {
    ...
    if(!IN_MULTICAST( ntohl(group) )) {
        igmplog(LOG_WARNING, 0, "The group address %s is not a valid Multicast group.",
            inetFmt(group, s1));
        return;
    }
    ...
    if(sourceVif == NULL) {
        igmplog(LOG_WARNING, 0, "No interfaces found for source %s",
            inetFmt(src,s1));
        return;
    }

    if(sourceVif->InAdr.s_addr == src) {
        igmplog(LOG_NOTICE, 0, "The IGMP message was from myself. Ignoring.");
        return;
    }
    ...
    if(sourceVif->state == IF_STATE_DOWNSTREAM) {
        ...
        IF_DEBUG igmplog(LOG_DEBUG, 0, "Should insert group %s (from: %s) to route table. Vif Ix : %d",
            inetFmt(group,s1), inetFmt(src,s2), sourceVif->index);

        // The membership report was OK... Insert it into the route table..
        insertRoute(group, sourceVif->index);

        // 2010/01/18 Adam & Jerry: for IPTV multicast streaming
        if(hasUpstreamIgmpInf) 
        {
            struct IfDesc  *Dp;
            Dp = getIfByName(extraInterface);
            IF_DEBUG igmplog(LOG_DEBUG, 0, "find %s interface, forward IGMP packets to %s\n", extraInterface, extraInterface);
            IF_DEBUG igmplog(LOG_DEBUG, 0, "JOIN sendto to %s from %s, INET: [%s]\n",
                             inetFmt(group, s1), inetFmt(Dp->InAdr.s_addr, s2), Dp->Name);
            sendIgmp(Dp->InAdr.s_addr, group, type, 0, group, 0);
        }
    } else {
        // Log the state of the interface the report was recieved on.
        igmplog(LOG_INFO, 0, "Mebership report was recieved on %s. Ignoring.",
...
void acceptLeaveMessage(uint32 src, uint32 group) {
    ...
    IF_DEBUG igmplog(LOG_DEBUG, 0, "Got leave message from %s to %s. Starting last member detection.",
    ...
    if(sourceVif->state == IF_STATE_DOWNSTREAM) {
        ...
        IF_DEBUG igmplog(LOG_DEBUG, 0, "The found if for %s was not downstream. Ignoring leave request.");
    }
	...
	if(hasUpstreamIgmpInf) 
	{
		...
		IF_DEBUG igmplog(LOG_DEBUG, 0, "find %s interface, forward IGMP packets to %s\n", extraInterface, extraInterface);
		IF_DEBUG igmplog(LOG_DEBUG, 0, "LEAVE sendto to %s from %s, INET: [%s]\n",
                         inetFmt(group, s1), inetFmt(Dp->InAdr.s_addr, s2), Dp->Name);

		croute = findRoute(group);
		if(croute!=NULL)
			sendIgmp(Dp->InAdr.s_addr, group, IGMP_V2_LEAVE_GROUP, 0, group, 0);
		removeRoute(croute);
	}
}
...
void sendGroupSpecificMemberQuery(void *argument) {
    ...
    IF_DEBUG igmplog(LOG_DEBUG, 0, "Sent membership query from %s to %s. Delay: %d",
...
void sendGeneralMembershipQuery() {
    ...
    for ( Ix = 0; Dp = getIfByIx( Ix ); Ix++ ) {
        // 2010/01/18 jerry chen: avoid igmpproxy sending membership query to upstream igmp interface
        if (strcmp(Dp->Name, extraInterface)!=0 && Dp->InAdr.s_addr && ! (Dp->Flags & IFF_LOOPBACK) ) {
