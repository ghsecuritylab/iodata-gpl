...
static inline int hub_is_superspeed_port(struct usb_hub *hub, int port1)
{
	...
	if (hdev->descriptor.bDeviceProtocol == 3) {
		if (hdev->parent != NULL)
			ret = 1;
		else if (hcd->chip_id == HCD_CHIP_ID_ETRON_EJ168
			&& port1 >= 3)
			ret = 1;
...
static unsigned get_port_link_stat(struct usb_hub *hub, int port1, unsigned portstatus)
{
	unsigned pls = 0;

	if (hub_is_superspeed_port(hub, port1))
		pls = portstatus & USB_PORT_STAT_LINK_STATE;
...
static void hub_activate(struct usb_hub *hub, enum hub_activation_type type)
{
 ...
 init2:
	...
	for (port1 = 1; port1 <= hdev->maxchild; ++port1) {
		...
		pls = get_port_link_stat(hub, port1, portstatus);
		if (pls == USB_SS_PORT_LS_POLLING ||
...
static void hub_events(void)
{
	...
	while (1) {
		...
		for (i = 1; i <= hub->descriptor->bNbrPorts; i++) {
			 ...
			 */
			pls = get_port_link_stat(hub, i, portstatus);
			if (pls == USB_SS_PORT_LS_SS_INACTIVE ||
				pls == USB_SS_PORT_LS_COMP_MOD ||
				pls == USB_SS_PORT_LS_POLLING) {
...
int usb_is_etron_hcd(struct usb_device *udev)
{
	...
	if (hcd->driver &&
		!strcmp(hcd->driver->description, "etxhci_hcd_120619"))
		return 1;
