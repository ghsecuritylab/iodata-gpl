...
#ifdef CONFIG_WPS2
/* Senao patch, 20130716 leonard: add for accept wps2.0 sta to connect WEP encrypt. */
#ifndef CONFIG_ATHEROS_DISABLE_WPS2_AT_STA
...
#endif
	if (!(wps->cred.encr_type &
	      (WPS_ENCR_NONE | WPS_ENCR_TKIP | WPS_ENCR_AES | WPS_ENCR_WEP)))
	{
						 ...
						 const struct wpabuf *msg)
{
	...
	case RECV_M6:
		...
		break;
	case RECV_M8:
		/*samhuang: 20120524 fix bug: the registra may learn our configuation. 
		  it will continue protocol run to successful completion.
		  And config_error will be 0.So give it success event */
		if(!WPA_GET_BE16(attr.config_error))
			wps_success_event(wps->wps);
		else
