...
static int get_mono(struct timespec *ts)
{
	...
	if (fd != NULL) {
		if (fgets(buffer, sizeof(buffer), fd) != NULL) {
			sscanf(buffer, "%ld.%ld", &(tv.tv_sec), &(tv.tv_usec));
			r = 0;
		}
		ts->tv_sec  = tv.tv_sec  * 1000000ULL;
...
static int calculate_timeout(struct usbi_transfer *transfer)
{
	...
	/*
	 * mook 2009-0413:
	 *   uClibc 0.9.29 don't support clock_gettime(),
	 *   we need to use a wrapper function get_mono()
	 *   from busybox.
	 */
#if 0
	r = clock_gettime(CLOCK_MONOTONIC, &current_time);
#else
	r = get_mono(&current_time);
#endif
	if (r < 0) {
