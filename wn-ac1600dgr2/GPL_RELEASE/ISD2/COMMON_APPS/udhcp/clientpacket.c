...
static void init_packet(struct dhcpMessage *packet, char type)
{
	...
	// 2013-10-31 Norkay, change sending packet's interface
	memcpy(packet->chaddr, client_config.ifidx_send ? client_config.arp_send : client_config.arp, 6);
    ...
    if(client_config.bootp_broadcast_flag)
    {
...
static void add_requests(struct dhcpMessage *packet)
{
	...
	for (i = 0; dhcp_options[i].code; i++)
	{
		...
		{
#if SUPPORT_IPV6_6RD
			if(client_config.option_6rd_enable == 0 && strcmp(dhcp_options[i].name,"6rd")==0) continue;
#endif
			if(client_config.classless_static_route_enable == 0 && strcmp(dhcp_options[i].name,"ms_csr") == 0) continue;
...
int send_discover(unsigned long xid, unsigned long requested)
{
	...
	if(client_config.ifidx_send != 0)
	{
		return raw_packet(&packet, INADDR_ANY, CLIENT_PORT, INADDR_BROADCAST,
					SERVER_PORT, MAC_BCAST_ADDR, client_config.ifidx_send);
	}
	return raw_packet(&packet, INADDR_ANY, CLIENT_PORT, INADDR_BROADCAST,
...
#if HAS_FIXED_MANAGEMENT_IP_IN_AP_MODE
	system("sysconf_cli inform release-management-ip");
#endif
...
int send_renew(unsigned long xid, unsigned long server, unsigned long ciaddr)
{
	...
	// 2013-10-31 Norkay, change sending packet's interface
	else if(client_config.ifidx_send != 0)
