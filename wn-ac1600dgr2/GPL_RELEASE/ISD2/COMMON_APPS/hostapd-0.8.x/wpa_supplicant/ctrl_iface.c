...
static int wpa_supplicant_global_iface_interfaces(struct wpa_global *global,
						  char *buf, int len);
static int transRSSI(int signal);
...
#if defined(SENAO_MOD)
			ret = os_snprintf(pos, end - pos, "essid=%s\nid=%d\n",
#else
...
#if WPA_SUPPLICANT_SCAN_RESULT_SHOW_NETWORK
static char * wpa_supplicant_network_txt(const struct wpa_bss *bss, const int channel)
{
	...
	rates_num = wpa_bss_get_bit_rates(bss, &ie_rates);
	htsupport = wpa_bss_get_ie(bss, WLAN_EID_HT_CAP);
	...
	const struct wpa_bss *bss, char *buf, size_t buflen)
{
...
#if WPA_SUPPLICANT_NO_SCAN_HIDDENSSID
	if(!strcmp(bss->ssid,""))
		return 0;
#endif
	...
	if (bss->freq < 4000)
	{
		channel = (bss->freq - 2407) / 5; //2.4g
	}
	else
	{
		channel = (bss->freq - 5000) / 5; //5g
	}

	ret = os_snprintf(pos, end - pos, MACSTR "\t%d\t%d\t",
			  MAC2STR(bss->bssid), channel, transRSSI(bss->level));
	if (ret < 0 || ret >= end - pos)
		return -1;
	pos += ret;
...
#if WPA_SUPPLICANT_SCAN_RESULT_SHOW_NETWORK
	ret = os_snprintf(pos, end - pos, "\t%s",
			wpa_supplicant_network_txt(bss, channel));
	if (ret < 0 || ret >= end - pos)
		return -1;
	pos += ret;
#endif
...
/*Adonn 08-0812: use ralink translate function. It isn't ok*/
static int transRSSI(int signal)
{
    unsigned int rssi_Quality=0;
#ifdef FOR_ATHEROS_PLATFORM
    /* Yolin 2012:1128: modified from LSDK-9.2.x */
    int rssi = signal;

    if(rssi >= 42)
        rssi_Quality = 94 ;
    else if(rssi >= 30)
        rssi_Quality = 85 + ((94-85)*(rssi-30) + (42-30)/2) / (42-30) ;
    else if(rssi >= 5)
        rssi_Quality = 5 + ((rssi - 5) * (85-5) + (30-5)/2) / (30-5) ;
    else if(rssi >= 1)
        rssi_Quality = rssi ;
    else
        rssi_Quality = 0 ;

    rssi_Quality = rssi_Quality*100/94; /* convert to percent */
    ...
    {
        rssi_Quality = rssi*2+((random()%2)?1:2);
    }
    else if (rssi > 0)
    {
        rssi_Quality = signal;
...
#if WPA_SUPPLICANT_SCAN_RESULT_SHOW_NETWORK
	ret = os_snprintf(pos, end - pos, "bssid / channel / signal level / "
			  "flags / ssid / network\n");
#else
	ret = os_snprintf(pos, end - pos, "bssid / channel / signal level / "
			  "flags / ssid\n");
